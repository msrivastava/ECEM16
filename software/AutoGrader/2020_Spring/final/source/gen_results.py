		...
		for i in range(num_sigs):
			...
			if 'x' in traces[i][t]:
				if not ('0' in traces[i][t] or '1' in traces[i][t]) or len(traces[i][t])<5:
					print("x*{0}".format(len(traces[i][t])),end="\t")
				else:
...
def default_sequential_grader(design_config, traces):
	...
	jobs_arrived = 0
	jobs_completed = 0
	errrec = {'reset':0, 'protocol':0, 'data':0, 'nonbinary':0, 'fatal':0}
	speedrec = dict()
	...
	for t in range(t_max):
		...
		for i,e in enumerate(design_config['pins']):
			...
			if sigrec[p]['val']!=sigrec[p]['val@1']:
				...
				sigrec[p]['t@1'] = t
		if t>0 and edge('REQ',t,sigrec):
			jobs_arrived = jobs_arrived+1
		if t>0 and edge('ACK',t,sigrec):
		...
		for e in design_config['assertions']:
			...
			if e['func'](design_config,t,sigrec,errrec,speedrec,msgs):
				...
				if errrec['fatal']==1:
					...
					errrecord = "{{reset={0}, protocol={1}, data={2}, nonbinary={3}, fatal={4}}}".format(
						errrec['reset'], errrec['protocol'], errrec['data'], errrec['nonbinary'], errrec['fatal'])
					errsummary = "Testing of {0}.{1} abandoned @ {2} due to error: {3}".format(
						design_config['dut'][0],design_config['dut'][1],t,err[-1])
					print("Error Record for {0}.{1}:\n\t{2}".format(design_config['dut'][0], design_config['dut'][1],errrecord))
					score = 0
					extra_credit = None
					return (err, errsummary, score, extra_credit)
	print("")
	print("Testing of {0}.{1} completed with {2} jobs arrived and {3} jobs finished correctly or incorrectly.".format(
		design_config['dut'][0], design_config['dut'][1], jobs_arrived, jobs_completed))
	errrecord = "{{reset={0}, protocol={1}, data={2}, nonbinary={3}, fatal={4}}}".format(
		errrec['reset'], errrec['protocol'], errrec['data'], errrec['nonbinary'], errrec['fatal'])
	errsummary = "{0}.{1}: {2}".format(design_config['dut'][0], design_config['dut'][1],errrecord)
	print("Error Record for {0}.{1}:\n\t{2}".format(design_config['dut'][0], design_config['dut'][1],errrecord))
	print("Speed Record: ")
	for k in speedrec:
	...
	score = default_scorer(jobs_arrived, jobs_completed, errrec, speedrec, design_config['maxscore'])
	print("Score = {0}/{1}".format(score,design_config['maxscore']))
	if 'extracreditfunc' in design_config and 'maxextracredit' in design_config and int(design_config['maxextracredit'])>0:
		extra_credit = design_config['extracreditfunc'](jobs_arrived, jobs_completed, errrec, speedrec, (design_config['maxextracredit']))
		print("Extra Credit for {0}.{1} = {2} / {3}".format(
			design_config['dut'][0],design_config['dut'][1],extra_credit,int(design_config['maxextracredit'])))
	else:
		extra_credit = None
		print("No Extra Credit Available.")
	return err,errsummary,score,extra_credit

def rising_edge(signame,t,sigrec):
	return sigrec[signame]['t@1']==t and sigrec[signame]['val']=='1'

def falling_edge(signame,t,sigrec):
	return sigrec[signame]['t@1']==t and sigrec[signame]['val']=='0'

def event(signame,t,sigrec):
	return sigrec[signame]['t@1']==t
...
def default_scorer(jobs_arrived, jobs_completed, errrec, speedrec, max_score, nonbinary_penalty=0.25, score_frac_reset=0.1, score_frac_protocol=0.2):
	...
	if errrec['reset']==0:
		score = score + score_frac_reset*max_score
	if errrec['protocol']==0:
		score = score + score_frac_protocol*max_score
	score = score + score_frac_data*max_score*(jobs_completed-errrec['data'])/jobs_arrived
...
def final_part2_ec(jobs_arrived, jobs_completed, errrec, speedrec, max_score_ec):
	if any([errrec[k]!=0 for k in errrec]):
		return 0
	if mean(speedrec['REQ2ACK+CorrectDOUT'])<14:
		return max_score_ec
	elif mean(speedrec['REQ2ACK+CorrectDOUT'])<20:
		return max_score_ec*0.8
	elif mean(speedrec['REQ2ACK+CorrectDOUT'])<30:
		return max_score_ec*0.6
	elif mean(speedrec['REQ2ACK+CorrectDOUT'])<40:
		return max_score_ec*0.4
	elif mean(speedrec['REQ2ACK+CorrectDOUT'])<50:
		return max_score_ec*0.2
	else:
		return 0
...
def writeResults(design_config_list,output_toplevel,output_tasklevel_list,scores,extra_credits=None):
	extra_credit_available = any([('maxextracredit' in e) and (int(e['maxextracredit'])>0) and ('extracreditfunc' in e) for e in design_config_list])
