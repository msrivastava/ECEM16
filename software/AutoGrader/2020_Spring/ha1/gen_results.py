...
def test_design(design_config,output_toplevel,output_tasklevel,scores,libList=libList):
	...
	if is_logisim_design and not logisim in allowed_logisim:
		scores.append(0)
		output_tasklevel.append("Design {0} uses Logisim version {1} which is not allowed.".format(design_name, logisim))
		output_toplevel.append("Invalid Logisim used in design {0}.".format(design_name))
		return
	elif is_logisim_design and not design_config['name'][1] in design.keys():
		scores.append(0)
		errmsg = "Didn't find design {0}.".format(design_name)
		...
		output_toplevel.append("Design {0} uses disallowed gate.".format(design_name))
		return
	else:
